...
def main():
    ...
    res = Image.open("results.png")
    st.image(res, caption="Training results.")
...
def run_the_app():
    ...
    if uploaded_file:
        ...
        pred = detect(img_array)
        st.image(pred, caption="prediction")
    ...
    for i, det in enumerate(pred):
        if len(det):
            ...
            for *xyxy, conf, cls_ in reversed(det):
                c = int(cls_)
                plot_one_box(xyxy, img_c, label='animal', color=colors(c, True), line_thickness=2)
    return img_c
